...
public class TextAnalyzer extends Configured implements Tool {
    ...
    public static class TextReducer extends Reducer<Text, Tuple, Text, Text> {
            ...
            throws IOException, InterruptedException
        {
            ...
            for(Tuple t : queryTuples){
                for(Text query : t){
                    contextWords.add(query); //add each query word into the aggregate tuple
                }
            }

            MapWriteable map = contextWords.getMap();
    ...
    public static class Tuple implements Writeable {

        private MapWriteable<Text, IntWriteable> map; //holds context word, count pairs

        public Tuple(){
            map = new MapWriteable<Text, IntWriteable>();
        }

        public void add(String word){
            if(map.containsKey(word)){ //if the word to enter already exists in the set, just increment count
                map.put(new Text(word), new IntWriteable(map.get(word).get() + 1));
            }
            else{
                map.put(new Text(word), new IntWriteable(1));
            }
        }

        public void add(Text word){
            if(map.containsKey(word)){ //if the word to enter already exists in the set, just increment count
                map.put(word, new IntWriteable(map.get(word).get() + 1));
            }
            else{
                map.put(word, new IntWriteable(1));
            }
        }

        public MapWriteable<Text, IntWriteable> getMap(){
            return map;
        }

        @Override
        public void write(DataOutput out) throws IOException{
            map.write(out); //serialize the map
        }        

        @Override
        public void readFields(DataInput in) throws IOException{
            map.readFields(in); //deserialise the map
        }

        @Override
        public boolean equals(Object o)
        {
            if(o instanceof MapWriteable)
            {
                return map.equals(o);
            }
